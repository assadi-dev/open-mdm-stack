...
description: >-
  ...
  authentification Better Auth, schéma et migrations Drizzle ORM sur PostgreSQL,
  configuration nodemon, structure du package backend dans le monorepo Turborepo.
  Exemples : « crée l'endpoint d'enrôlement de device », « configure Better Auth
  (email/password, plugin JWT) », « définis le schéma Drizzle des devices et policies ».
...
- **Better Auth** — authentification (email/password, sessions, plugin JWT) via l'adapter Drizzle. Déjà scaffolé dans `src/lib/auth.ts`.
...
Better Auth et ses plugins, pg, etc.), **récupère la doc à jour via le MCP
...
5. Sécurité : ne hardcode jamais de secrets (DATABASE_URL, BETTER_AUTH_SECRET, JWT_SECRET via env). Le hash des mots de passe est géré par Better Auth — ne le réimplémente pas. Valide les entrées.
